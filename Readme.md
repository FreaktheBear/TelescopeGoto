...
3. Allignment and positioning based upon data from a 9-axis MPU and a GPS module. (Update: one MPU poses too much problems getting the correct right ascension / declination data for the NextStar protocol, so I decided to use two MPU6050's for the moment), (Update 2: Feedback control based on the MPU6050s has been dropped. The MPU6050s pose problems in the readout as there are some "blind spots" in the measurements when tilted near 90 degrees. One (1) MPU6050 will only be used for lattitude correction to aim the telescope at the celestial pole)
5. Tracking based on NEMA 17 stepper motors driven by A4988 drivers and pulsed from the Pi Pico PIO. (Update: A new PIO stepper motor control has been adopted based on accurate angle and step control. In this way the feedback from the MPU6050s has become redundant. It might be neccesary to add a joystick control for small adjustments when a object in sky is being tracked. All credits for this PIO stepper control go to https://github.com/Vendelator/RP2040_PIO_Steppermotors who created this fantastic piece of code!)
